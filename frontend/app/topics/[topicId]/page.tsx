...
import { topicApi, videoApi } from '@/lib/api';
...
export default function TopicPage() {
  ...
  const fetchTopic = async () => {
    try {
      const topicData = await topicApi.getTopic(topicId);
      setTopic(topicData);
    } catch (error) {
  ...
  const fetchVideos = async () => {
    try {
      setLoading(true);
      const { videos: videosData } = await topicApi.getVideosByTopic(topicId);
...
function AddVideoModal({ topicId, topicName, onClose, onVideoAdded }: AddVideoModalProps) {
  const router = useRouter();
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Process video with topic_id
      const response = await videoApi.processVideo(videoUrl, undefined, topicId);
      ...
      await supabase.from('activity_log').insert({
        ...
        activity_desc: `Added new video to topic "${topicName}"`,
